...
public class DrunkardsWalkManual : MonoBehaviour
{
    private int[,] map;
    ...
    void Start(){
        ...
        ResetFootsteps();
        SimulateIteration((int) Mathf.Floor(size/2), (int) Mathf.Floor(size/2));
        AddNewFootsteps(index+1);
        ShowStepColors();
    }
        ...
        for(int i = 0; i < size; i++){
            for(int j = 0; j < size; j++){
                if(map[i,j] == 0){
                    ...
                    mapGO[i,j].GetComponent<SpriteRenderer>().color = new Color32(0,0,0,255);
                }
                else{
